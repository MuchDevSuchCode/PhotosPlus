# Polish review — 2026-08-08 (HEAD `034e0bc`)
...
- [ ] **P0-1 Drives are not excluded from Delete / Shift+Delete / Shred.** `MainWindow.xaml.cs`
...
- [ ] **P0-2 "Send to Vault" shreds originals even when their import FAILED.** `Vault.cs:355-398`
...
- [ ] **P0-3 Remote-browse path traversal (arbitrary file write).** `MainWindow.SecureSharing.cs:648,670`:
...
- [ ] **P0-4 VaultManager is per-window ⇒ double-unlock wipes the live working folder.** `MainWindow.xaml.cs:104`
...
- [ ] **P0-5 `Vault.LockAsync` wipes the working folder in `finally` even when the commit threw.**
...
- [ ] **P1-1 Video mute memory is erased before it's read** *(regression from `e4f5160`)*.
...
- [ ] **P1-2 "Start videos muted" contaminates the persisted state and mutes audio files**
...
- [ ] **P1-3 `RefreshFolderIncremental` desyncs `_explorerRaw` from `_explorerItems` ⇒ rename visibly reverts.**
...
- [ ] **P1-4 Recycle-bin index is non-atomic and unsynchronized.** `RecycleBin.cs:75-94`: store move happens
...
- [ ] **P1-5 Deleting a vault file puts decrypted plaintext in the plain recycle bin.** Delete paths call
...
- [ ] **P1-6 Recursive search ignores `HiddenFolders` (and the hidden-items toggles).**
...
- [ ] **P1-7 Path-keyed state is never migrated on rename.** `HiddenFolders`, `FolderThumbnails`,
...
- [ ] **P1-8 FileTransfer fast-move: no in-batch destination guard, no fallback.** `FileTransfer.cs:109-129`:
...
- [ ] **P1-9 Clipboard `_suppressClipChange` is a fragile one-shot.** Zero `ContentChanged` events leaves the
...
- [ ] **P1-10 Drag & drop defaults to MOVE even cross-volume.** `MainWindow.xaml.cs:4342`: Ctrl-only decision;
...
- [ ] **P1-11 Bulk rename can strand files as `__galileo_<guid>` with no recovery.** Phase-1 temp renames have
...
- [ ] **P1-12 F2 / Ctrl+X inside the Recycle Bin corrupts the store.** Keyboard paths aren't gated on
...
- [ ] **P1-13 `OpenViewerDirect` sibling backfill has no generation token.** Slow folder + quickly opening a
...
- [ ] **P1-14 Opening a Hidden-album photo shows the wrong photo (or an empty window).** Hidden photos still
...
- [ ] **P1-15 Slideshow: three defects.** `SlideshowWindow.xaml.cs` — (a) no generation token in `ShowAtAsync`
...
- [ ] **P1-16 Re-unlocking an already-unlocked vault wipes its live working folder.** `VaultManager` (56)
...
- [ ] **P1-17 Failed unlock leaves unowned plaintext + corrupt blob counts as a wrong passphrase.**
...
- [ ] **P1-18 Secure wipe follows junctions/symlinks; cancel leaves zeroed files that look intact.**
...
- [ ] **P1-19 Backing up an unlocked vault can produce an unrestorable backup.** `GoogleDriveBackup.cs:144-170`:
...
- [ ] **P1-20 Tray "Exit" can be silently swallowed.** `AppWindow_Closing` `CloseToViewerBack` branch (~6716)
...
- [ ] **P1-21 Guest/`--new-window` windows still wipe shared state.** (a) `:6744` guards only
...
- [ ] **P1-22 Process-global sort state bleeds between windows.** `_state.SortBy/SortDescending/GroupBy` act
...
- [ ] **P1-23 `state.json` writes are non-atomic and multi-process-unsafe.** `AppState.Save` is
...
- [ ] **P1-24 Editor: stale selection mask after Reset.** `EditReset_Click` (Editor.cs ~1176) doesn't
...
- [ ] **P1-25 Video editor survives into image mode armed with a stale path.** `EnterImageMode` doesn't
...
- [ ] **P1-26 Failed overwrite leaks `.galileo-tmp` that lists as a photo.** Editor.cs ~1290: export-failure
...
- [ ] **P1-28 Editor re-activation timer can steal foreground from another app** *(from `9ed3db2`)*.
...
- [ ] **P1-29 `AiSelectText_Click` lacks the `_aiGeneration` guard** — a slow text-detect finishing after the
  user switched photos installs photo A's mask on photo B. *Fix:* same generation check as `RunAiAsync`.

- [ ] **P1-30 Deferred watcher refresh never consumed when returning via `ShowExplorer`** *(from `51ce10d`)*:
...
- [ ] **UI-1 Light theme: title bar, filename/counter, status text and caption buttons are invisible in
...
- [ ] **UI-2 Privacy curtain leaks metadata.** The "Eye" black curtain renders *below* `ViewerChrome` and
...
- [ ] **UI-3 Mica has no fallback** — no `MicaController.IsSupported()` check and `RootGrid.Background = null`;
...
- [ ] **UI-4 Zero `AutomationProperties` app-wide** — ~50 icon-only buttons announce as "button", all sliders
...
- [ ] **UI-6 Explorer command strip drops commands at ordinary widths** — single non-wrapping StackPanel row;
...
- [ ] **UI-7 Editor command bar physically overlaps below ~660 px width** — three panels in one Grid cell
  aligned L/C/R; Save-As draws over Redo.
  *Fix:* real `Auto,*,Auto` columns.

- [ ] **UI-8 Settings card: fixed 540 px width; height cap computed once at open** — shrink the window and
  Save/Cancel go off-screen (remaining exits discard changes, see UI-11).
  *Fix:* `MaxWidth` + re-cap on `SizeChanged`.

- [ ] **UI-9 Viewer chrome fades under a stationary cursor and stays in tab order while invisible** —
...
- [ ] **UI-10 Peek/Settings overlays aren't focus-modal** — Tab walks into the dimmed explorer behind the
...
- [ ] **UI-11 Scrim click discards all settings silently** — the card has explicit Save/Cancel, but a stray
  click 2 px outside reverts everything with no prompt.
  *Fix:* no-op (or confirm) on scrim tap when changes are pending.

## P2 — Minor

- [ ] **P2-1 `ExplorerItem.Rename` doesn't refresh `TypeName`/icon** — rename `.txt`→`.jpg` keeps the TXT
  glyph and sorts/groups as TXT until a full reload. *Fix:* recompute TypeName + `ResetIcon()` when the
  extension changes.
- [ ] **P2-2 Current folder deleted externally ⇒ permanently stale listing** (watcher tears itself down;
  incremental refresh returns early). *Fix:* navigate to nearest existing ancestor with a status note.
- [ ] **P2-3 Ctrl+marquee wipes the pre-existing selection** — baseline the selection on press and don't
  remove items outside the marquee that were in the baseline.
- [ ] **P2-4 Photo-window placement clamps X/Y but not W/H** — a 4K-sized window restored on a 1080p panel
...
- [ ] **P2-5 Back from a directly-opened video lands on This PC** — `ShowExplorer` seeds from `Current?.Path`
  (null in video mode); use `_currentVideoPath` too.
- [ ] **P2-6 F5 during video playback starts a slideshow on top of the playing audio** — guard `InVideo` or
  pause first.
- [ ] **P2-7 Peek: Space on a focused button also opens Peek** (`handledEventsToo` ignores `e.Handled`);
  PeekNavigate steps onto drives it can't preview; PeekVideo ignores the remembered mute/volume.
- [ ] **P2-8 Volume-save debounce not flushed on close** (change volume then close within 600 ms ⇒ lost).
- [ ] **P2-9 Lasso combine semantics** *(from `7539983`)*: intersect with no prior selection should clear
...
- [ ] **P2-10 Filmstrip temp dir leaks when the video editor closes during thumbnail generation.**
- [ ] **P2-11 Trim end not validated against start** ⇒ exports a 0.01 s clip as success.
- [ ] **P2-12 Markup stroke width/text size baked from current zoom** — visually identical arrows export at
  wildly different weights depending on zoom at draw time. *Fix:* derive from image dimensions.
- [ ] **P2-13 Remote vault deletes use plain `File.Delete`** (not `OverwriteAndDelete`); `_uploads` dictionary
  isn't thread-safe across peers; upload part names collide.
- [ ] **P2-14 Drive restore silently overwrites a newer local vault; trusts remote names** — confirm
  overwrite; sanitize names.
- [ ] **P2-15 Audit window: never closed by main close ⇒ headless process keeps polling relay; tray toggles
  from a guest window create a second tray icon.**
- [ ] **P2-16 Drop onto MTP/device targets runs the filesystem engine** ("Copied 0, N failed") instead of
  routing to `_shell.Upload`.
- [ ] **P2-17 FileTransfer conflict checks ignore directories at the destination** (`File.Exists` only) ⇒
  copy onto a same-named folder throws per-file instead of prompting.
- [ ] **P2-18 Tab-switch suppression flag can desync TabView selection and history** (`_suppressNextTabSelection`
  armed after `_switchingTabs` cleared).
- [ ] **P2-19 Search on This PC with no matches shows a blank pane** (empty-state requires a folder).
- [ ] **P2-20 View-mode switch (Icons↔Details) drops the selection** — two ListViews, selection not copied over.
- [ ] **P2-21 Truncated filenames/status have no tooltips anywhere** (StatusText is even hit-test-invisible).
- [ ] **P2-22 Details columns: duplicated magic widths, no Name MinWidth** — name column collapses first.
- [ ] **P2-23 Video chrome never auto-hides** (back/volume pills sit over the frame; photo chrome fades).
- [ ] **P2-24 "Hide folder" tooltip goes stale** — says "Hide" while the button reads "Unhide".
- [ ] **P2-25 Slideshow Prev/Play/Next lack tooltips; Play tooltip never flips; chrome hides via hard
  Collapse mid-hover.**
- [ ] **P2-26 Transfer progress is a hand-drawn Border** — no automation RangeValue, hardcoded blue gradient
  ignores the Terminal/Gray themes. *Fix:* real `ProgressBar`.
- [ ] **P2-27 Fixed pixel row heights clip at Windows text scaling ≥150%** (details rows, sidebar rows,
...
- [ ] **P3-1 Icon vocabulary collisions** — same glyph for Collage and View-mode; the two adjacent
  hidden-items toggles use opposite eye metaphors; the Eye flyout inverts the app's own convention.
- [ ] **P3-2 `FolderSorts` grows forever** (dead entries never pruned).
